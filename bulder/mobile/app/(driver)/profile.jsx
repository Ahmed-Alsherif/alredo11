    ...
    <SafeAreaView style={styles.safe}>
      <ScrollView style={styles.container} contentContainerStyle={{ paddingBottom: 32 }}>
        <View style={styles.header}>
          <Text style={styles.title}>حساب السائق</Text>
          <Text style={styles.subtitle}>بيانات العمل، المنطقة، وإعدادات الدخول.</Text>
        </View>

        <View style={styles.profileCard}>
          <View style={styles.avatar}>
            <Text style={styles.avatarText}>{(user?.first_name || user?.username || 'س').charAt(0)}</Text>
          </View>
          ...
          <View style={styles.statusPill}>
            <Ionicons name="car-outline" size={16} color={Colors.roles.driver} />
            <Text style={styles.statusText}>{user?.zone_name ? `مسار ${user.zone_name}` : 'سائق نشط'}</Text>
          </View>
        </View>
  ...
  return (
    <TouchableOpacity style={styles.menuItem} onPress={onPress}>
      <View style={styles.menuLeft}>
        <Ionicons name={icon} size={20} color={Colors.roles.driver} />
...
const styles = StyleSheet.create({
  ...
  avatar: { width: 78, height: 78, borderRadius: Radius.full, backgroundColor: Colors.roles.driver, justifyContent: 'center', alignItems: 'center', marginBottom: Spacing.sm },
  ...
  statusPill: { flexDirection: 'row-reverse', alignItems: 'center', gap: 6, backgroundColor: Colors.roles.driverSoft, paddingHorizontal: 14, paddingVertical: 7, borderRadius: Radius.full, marginTop: Spacing.md },
  statusText: { color: Colors.roles.driver, fontSize: FontSize.xs, fontWeight: '800' },
  ...
  saveBtn: { backgroundColor: Colors.roles.driver, paddingVertical: 14, borderRadius: Radius.md, alignItems: 'center', marginTop: Spacing.sm },
